...
@Component
class CheckBidStateHandler(
    val transform: Transform,
    logger: Logger,
    private val bidService: BidService
) : AbstractValidationHandlerV2<CommandTypeV2, Fail>(logger = logger) {

    override val action: CommandTypeV2 = CommandTypeV2.CHECK_BID_STATE

    override fun execute(node: JsonNode): Validated<Fail> {
        val params = node.tryGetParams(CheckBidStateRequest::class.java, transform = transform)
            .onFailure { return Validated.error(it.reason) }
            .convert()
            .onFailure { return Validated.error(it.reason) }

        return bidService.checkBidState(params)
    }
}